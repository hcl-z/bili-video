...
export function updateRoutes(deps: ServerDeps, poll: Poller): Hono {
  return new Hono()
    .get('/', (c) => {
      ...
      for (const s of deps.repos.subscriptions.list()) {
    ...
    .get('/:dynId', (c) => {
      const update = deps.repos.updates.get(c.req.param('dynId'))
      if (update === null) return c.json(errorBody('not-found', '库里没有这条动态'), 404)
      const body: ReaderItemResponse = { item: fromUpdate(deps, update) }
      return c.json(body)
    })
}
