...
/**
 * 三类故障的判定。纯函数，因为「什么算故障」和「告警发不发」是两件事：
 * 前者要能单独摆出来看，后者是编排（见 app/health.ts）。
 *
 * 只判这三类，因为只有它们会让系统**静默地不干活**：登录掉了、拉不到动态、
 * 转写一直炸。AI 调用失败不在内，那条路有降级链兜着，总结照样出得来。
 */
...
export function detectFaults(
  ...
  // auth-lost 是终态：cron 已经被摘掉，不催人重新登录的话系统就这么静静躺着。
