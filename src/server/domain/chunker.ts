...
/** 按 token 阈值把字幕切成段。纯函数：同样的入参永远同样的段。 未超阈值时返回**一段**（调用方据此走整篇总结）；超了才真的分。 切点永远落在字幕条边界上，不会把一句话切两半 */
export function chunkCues(
  ...
  while (start < items.length) {
    let end = start
    let acc = 0
    // 第单条无条件收进来：单条就超 sizeTokens 也得有个归宿，否则这里会空转
...
/** 把切点挪到 (min, end] 里间隔最大的那个停顿上。一个停顿都没有就留在 end —— 那仍然是字幕条边界，只是不好看 */
...
/** 下一段的起点：从 end 往回退够 overlapTokens 的量，再挪到停顿上。 重叠是为了让跨段的一句话在两边都完整出现，所以它的起点同样不应落在句子中间。 floor 保证每轮至少前进单条，不然重叠会把游标拽回原地 */
