...
import type { BiliRelationWriter } from '../../types/bili.ts'
...
import type { WriteAuditRepo } from '../../types/persistence.ts'
...
export interface WriteLimits {
  /** 总开关。false = 一个写请求都不发。 账号真的开始吃 -352 的时候得有地方踩刹车，而 `maxPerHour` 最小是 1、关不掉。 spec 把「自动关注的风控表现」列为未测风险，未测的数据就得能关 */
  autoFollow: boolean

  minIntervalMs: number
  /** 一小时最多几次写调用。数的是审计表，所以重启不会无效送额度 */
  maxPerHour: number
}
...
/** 全系统唯一的写接口适配器。 三件事只在这里做，避免处都不许：查关系（读，不限流）、关注（写，限流 + 审计）、 以及「已经关注了就不发写请求」—— 后者由调用方按 getRelations 的结果决定， 但限流和留痕必须在这一层兜住，否则换个调用方就绕过去了 */
export class BiliRelationsClient implements BiliRelationWriter {
  ...
  async getRelations(uids: string[]): Promise<Result<Map<string, boolean>>> {
    ...
    for (let i = 0; i < uids.length; i += BATCH_SIZE) {
      ...
      // 当失败则会在「新号一个都没关注」这个首次使用的场景里把功能整个阻塞
  ...
  /** 关注一个人。**全过程串行**：限流读的是审计表，而审计行要等请求回来才落 —— 并发进来的两次调用会各自读到「额度无效、也没人刚写过」，然后一起发出去。 撞得上：启动期补关注和页面上一次新增订阅，或者两个标签页各点一次「重试关注」。 队列本身就是限流的一部分，不是性能优化 */
  ...
  private async followOne(uid: string): Promise<Result<void>> {
    ...
    if (csrf === null) {
      // 没有 bili_jct 就没登录。这时候发出去必然 -111，无效记一笔风控
