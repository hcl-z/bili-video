...
/**
 * Node 24 内置 node:sqlite —— 零原生编译，镜像干净（放弃 better-sqlite3 就是为这个）。
 * 它是同步 API，所以整个仓储层也是同步的，不假装有异步边界。
 */
...
/** node:sqlite 的行是 Record<string, SQLOutputValue>；取值处收窄一次，别在每个仓储里 as。 */
