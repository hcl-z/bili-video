...
import type { BiliAuth } from '../types/bili.ts'
...
/** 一次登录最多换两张码；再不扫就应便于使用重新来一次，而不是无限刷 */
...
export class AuthLifecycle {
  ...
  /** 从页面上开一轮扫码。不 await 登录本身 —— 扫码要等人，HTTP 请求不应挂在应处。 页面之后靠 /api/system 的 state 和 qrUrl 看进展（登录态变化都会发 SSE 事件） */
  ...
  /** 手动续期。不走 decideAuthAction —— 那是「应不应续」，这里是人已经点了「续一下」。 续不动的原因照原样返回，页面要显示它；成功后剩余有效期由快照给出 */
  ...
  /** 核对并在需要时续期。启动时跑一次，之后由 cron 定期跑。 返回做了什么，而不是 void：调用方（cron）要据此决定要不要停轮询 */
  ...
  /** 出码 → 打印 → 轮询到终态。判据 9 的应项链就是这个方法。 轮询用注入的时钟 sleep，所以测试里推假响应就能走完全程，不用真等 */
  async loginByQr(): Promise<Result<AuthSnapshot>> {
    // 一次只出单个二维码：启动流程和页面上的「重新扫码」遇到时，后来的那个直接被挡回去
    if (this.loggingIn) {
