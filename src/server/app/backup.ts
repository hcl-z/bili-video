...
import type { FilterRuleRepo, SubscriptionRepo, SummaryRepo, UpdateRepo } from '../ports/repo.ts'
...
/**
 * 备份里**没有** master key，也没有任何用它加密过的东西（cookie、apiKey、refresh_token）：
 * 一份能被随手丢进网盘的文件不该带着 SESSDATA 走。
 */
...
export interface BackupFile {
  ...
  appVersion: string
  note: string[]
  /** 全库条数。和数组长度不一致就说明撞上了导出上限。 */
  counts: { updates: number; summaries: number }
  config: AppConfig
  subscriptions: Subscription[]
  rules: FilterRule[]
  updates: Update[]
  /** transcript 一起带上：少了它，恢复之后每条都得重跑一遍转写。 */
